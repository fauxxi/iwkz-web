...
const PeranIwkz = () => {
  let peranIwkz = [
    ...
    },
    {
      ...
      content: "mendorong keberhasilan studi mahasiswa Indonesia di Jerman, terutama di Berlin"
    },
    ];
  ...
  const listPeran = () =>{
    ...
    return(
      peranIwkz.map(item => {
        ...
        if(index % 2){
          return(
          <section className="section is-medium " id={"section-"+index} key={"peran"+index}>
  ...
  return (
        <div className="has-background-light section is-medium">
          {listPeran()}
        </div>
  );
};
